...
// Check and reset expired subscription (30 days)
checkAndResetExpiredSubscription($userId);

// Fetch user data after potential reset
    ...
    <style>
        * {
        ...
        @keyframes floatBlob {
            ...
            100% {
                transform: translate(8%, 12%) rotate(6deg) scale(1.15);
            }
        }
...
<body>
    ...
    <div class="max-w-7xl mx-auto px-4 py-6 sm:py-8 md:py-10">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
            <div>
                <h1
                    class="text-3xl sm:text-4xl font-extrabold bg-gradient-to-r from-purple-300 to-fuchsia-400 bg-clip-text text-transparent">
                    <i class="bi bi-grid-3x3-gap-fill"></i> Pawn Dashboard
                </h1>
                <p class="text-gray-400 text-sm mt-1"><i class="bi bi-database"></i> Manage your stored credentials &
                    data</p>
            </div>
            <div class="glass-card rounded-full px-5 py-2 flex items-center gap-2">
                <i class="bi bi-file-text-fill text-purple-300"></i>
                <span class="text-sm text-white/80 font-medium" id="recordCount"><?= count($pawnEntries) ?>
                    records</span>
                <i class="bi bi-wallet-fill text-purple-300"></i>
                <span class="text-sm text-white/80 font-medium" id="recordCount"><?= $limit ?>
                    Limit </span>
            </div>
        </div>
        ...
        <div class="glass-card rounded-2xl p-4 mb-6">
            <div class="flex flex-wrap gap-2 items-center">
                <span class="text-gray-300 text-sm font-medium"><i class="bi bi-tag-fill mr-1"></i>Filter by
                    Page:</span>
                <button class="page-pill px-4 py-2 rounded-full text-sm font-medium transition-all"
                    style="background: linear-gradient(105deg, #8B5CF6, #C084FC); color: white;" data-page="all">
                    <i class="bi bi-grid-3x3-gap-fill"></i> All
                </button>
                <?php foreach ($pages as $page): ?>
                    <button class="page-pill px-4 py-2 rounded-full text-sm font-medium transition-all"
                        style="background: rgba(255,255,255,0.1); color: #d1d5db;"
                        data-page="<?= htmlspecialchars(strtolower($page)) ?>">
                ...
                <?php if (!$hasLocation): ?>
                    <button class="page-pill px-4 py-2 rounded-full text-sm font-medium transition-all"
                        style="background: rgba(255,255,255,0.1); color: #d1d5db;" data-page="location">
                        <i class="bi bi-geo-alt-fill"></i> Locations
                    </button>
                <?php endif; ?>
                <?php if (!$hasImage): ?>
                    <button class="page-pill px-4 py-2 rounded-full text-sm font-medium transition-all"
                        style="background: rgba(255,255,255,0.1); color: #d1d5db;" data-page="image">
                        <i class="bi bi-image-fill"></i> Images
                    </button>
                <?php endif; ?>
                <button class="page-pill px-4 py-2 rounded-full text-sm font-medium transition-all"
                    style="background: rgba(255,255,255,0.1); color: #d1d5db;" data-page="credentials">
        ...
        <div class="glass-card rounded-2xl p-4 mb-8">
            <div class="flex flex-wrap gap-4 items-center">
                <div class="flex-1 relative">
                    ...
                    <input type="text" id="searchInput" placeholder="Search by email, password, content, or page..."
                        class="w-full pl-10 pr-4 py-2 bg-black/40 border border-purple-500/30 rounded-xl text-sm text-white placeholder-gray-400 focus:outline-none focus:border-purple-400">
                </div>
                <div class="flex gap-2">
                    <button id="clearFilters"
                        class="px-4 py-2 rounded-xl bg-white/10 text-gray-300 hover:bg-white/20 transition-all text-sm">
        ...
        <div id="cardsContainer" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php foreach ($pawnEntries as $entry): ?>
                ...
                ?>
                <?php if ($limit < 0): ?>
                    <div onclick="window.location.href = '../upgrade/'" class="pawn-card rounded-2xl p-5 transition-all duration-200 flex flex-col relative cursor-pointer"
                        ...
                        <!-- Header with page logo and badge -->
                            <i class="bi bi-lock-fill z-10 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white text-[5em]"></i>
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-12 h-12 rounded-xl <?= $pageColor ?> bg-opacity-30 flex items-center justify-center border border-white/20 shadow-lg">
                            ...
                            <span
                                class="text-xs px-2 py-1 rounded-full flex items-center gap-1 <?= $isLocation ? 'bg-red-900/50 text-red-300' : ($isImage ? 'bg-green-900/50 text-green-300' : 'bg-yellow-900/50 text-yellow-300') ?>">
                                <i
                                    class="<?= $isLocation ? 'bi bi-geo-alt-fill' : ($isImage ? 'bi bi-image-fill' : 'bi bi-key-fill') ?>"></i>
                                <?= $isLocation ? 'Location' : ($isImage ? 'Image' : 'Credentials') ?>
                            </span>
                        </div>

                        <!-- Content based on type -->
                        <div class="mb-4 blur-sm">
                            <?php if ($isLocation): ?>
                                <!-- Location Page -->
                                <div class="mb-3">
                            ...
                            <?php elseif ($isImage): ?>
                                ...
                                <div class="mb-3">
                                    ...
                                    <a href="" data-lightbox="gallery"
                                        data-title="Image from <?= htmlspecialchars($pageName) ?>">
                                        <img src="" alt="Gallery image"
                                            class="gallery-img rounded-xl border border-purple-500/30"
                                            onerror="this.src='https://placehold.co/400x300/1a1a2e/8B5CF6?text=Image+Preview+Not+Available'">
                                    </a>
                                </div>
                            ...
                            <?php else: ?>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div
                                        class="bg-black/30 rounded-xl px-3 py-2 font-mono text-sm text-white break-all flex justify-between items-center">
                                        <span id="">••••••••</span>
                                        <button onclick="togglePassword(<?= $entry['id'] ?>, '<?= addslashes($credPassword) ?>')"
                                            class="text-purple-400 hover:text-purple-300 ml-2">
                                ...
                                <?php if (!empty($newPassword)): ?>
                                    <div class="mb-3">
                                        ...
                                        <div
                                            class="bg-black/30 rounded-xl px-3 py-2 font-mono text-sm text-white break-all flex justify-between items-center">
                                            <span id="">••••••••</span>
                                            <button onclick="toggleNewPassword(<?= $entry['id'] ?>, '<?= addslashes($newPassword) ?>')"
                                                class="text-purple-400 hover:text-purple-300 ml-2">
                                                <i class="bi bi-eye-fill"></i>
                                            </button>
                                        </div>
                        ...
                        <div class="flex justify-between items-center gap-3 mt-auto pt-3 border-t border-purple-500/20">
                            ...
                            <div class="flex gap-2">
                                <?php if ($isLocation): ?>
                                    <button
                                        class="btn-secondary-copy text-white w-8 h-8 rounded-lg flex items-center justify-center transition-all"
                ...
                <div class="pawn-card rounded-2xl p-5 transition-all duration-200 flex flex-col"
                        ...
                        data-type="<?= $isLocation ? 'location' : ($isImage ? 'image' : 'credentials') ?>"
                        data-search="<?= htmlspecialchars($searchText) ?>"
                        data-password="<?= addslashes($entry['password']) ?>">
                        ...
                        <div class="mb-4">
                            <?php if ($isLocation): ?>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="bg-black/30 rounded-xl px-3 py-2 font-mono text-sm text-white break-all">
                                        <i class="bi bi-link-45deg text-purple-400 mr-1"></i>
                                        <a href="<?= htmlspecialchars($locationUrl) ?>" target="_blank"
                                            class="text-purple-400 hover:underline">
                            ...
                            <?php elseif ($isImage): ?>
                                ...
                                <div class="mb-3">
                                    ...
                                    ?>
                                    <a href="<?= htmlspecialchars($fullImagePath) ?>" data-lightbox="gallery"
                                        data-title="Image from <?= htmlspecialchars($pageName) ?>">
                                        <img src="<?= htmlspecialchars($fullImagePath) ?>" alt="Gallery image"
                                            class="gallery-img rounded-xl border border-purple-500/30"
                                            onerror="this.src='https://placehold.co/400x300/1a1a2e/8B5CF6?text=Image+Preview+Not+Available'">
                                    </a>
                                </div>
    ...
    <script>
        ...
        if (fabExport) {
            fabExport.addEventListener('click', (e) => {
                e.stopPropagation();
                closeFabMenu();
                let visibleCards = Array.from(document.querySelectorAll('#cardsContainer .pawn-card:not([style*="display: none"])')).filter(card => !card.querySelector('.blur-sm'));
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: rgba(139, 92, 246, 0.2);">
                                ...
                                <th style="padding: 10px; border: 1px solid rgba(139, 92, 246, 0.3); text-align: left;">Password/Data</th>
            ...
            `;
                ...
                visibleCards.forEach(card => {
                    ...
                    let passwordValue = card.dataset.password || '';
                    ...
                    <tr>
                        ...
                        <td style="padding: 10px; border: 1px solid rgba(139, 92, 246, 0.3); font-family: monospace;">${passwordValue}</td>
